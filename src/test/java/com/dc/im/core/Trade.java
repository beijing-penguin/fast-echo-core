 ...
 */
public class Trade {
    private static Logger LOG = LoggerFactory.getLogger(LoggerName.SEND);
    ...
    //线程池使用结论，保持线程池个数和cpu内核一致
    static ExecutorService serviceLOG = new ThreadPoolExecutor(10000, 20000, 10, TimeUnit.SECONDS, new LinkedBlockingQueue<Runnable>());
    public void useLog() throws Exception {
        CountDownLatch start = new CountDownLatch(COUNT);
        long begin = System.currentTimeMillis();
        for (int i = 0; i < COUNT; i++) {
            int ii = i;
            serviceLOG.submit(new Runnable() {
                @Override
                public void run() {
                    LOG.info(ii+"=java--测试写文件 fos");
                    start.countDown();
                }
            });
        }
